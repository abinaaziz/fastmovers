...
<%
   ...
   String query="SELECT status FROM tbl_site_register WHERE status =0";
    
    ResultSet rs=obj.select(query);
    if(rs.first()){
    int c =Integer.parseInt( rs.getString(1));
    if (c==0){
        out.print("<script>alert('New Registartions Added')</script>");
        out.print("<script>window.location.href='SiteChooser.jsp'</script>");
    }}
%>
